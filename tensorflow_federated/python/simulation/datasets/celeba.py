...
def load_data(split_by_clients=True, cache_dir=None):
  ...
  *split_by_clients=True*:
  ...
  -   train: 8,408 clients, X total examples
  -   test: 935 clients, X total examples

  *split_by_clients=False*:

  -   train: 9,343 clients, X total examples
  -   test: 9,343 clients, X total examples
  ...
  database_path = download.get_compressed_file(
      origin='https://storage.googleapis.com/tff-datasets-public/celeba_all.sqlite.lzma',
      cache_dir=cache_dir)
  if split_by_clients:
