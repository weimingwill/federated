...
export LD_LIBRARY_PATH="/mnt/lustre/share/cuda-10.1/lib64:$LD_LIBRARY_PATH"
 ...
 #-w SG-IDC2-10-51-3-50 \
srun -u --partition=innova --job-name=tff-cifar \
    -n8 --gres=gpu:8 --ntasks-per-node=8 -w SG-IDC2-10-51-3-38 \
    python -u ${root_dir}/tensorflow_federated/python/examples/simple_fedavg/fedavg_main.py \
        --data_dir ${data_dir}/cifar10/cifar10_iid_100_10_1_0.5_0/ \
        --dataset cifar10 --total_rounds 100 --train_clients_per_round 10 --client_epochs_per_round 10 \
        --batch_size 64 --test_batch_size 64 --client_learning_rate 0.01 --test_in_server | tee log/tff-cifar10-small-clean-${now}.log &
