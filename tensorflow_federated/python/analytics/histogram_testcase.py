...
class HistogramTest(tf.test.TestCase):
  ...
  def create_np_histogram(self, keys, values):
    """Create a numpy dictionary by the input keys and values.

    Args:
      keys: A 1D tensor of tf.string or list of strings containing the keys of
        the histogram.
      values: A 1D tensor or list of ints containing the values of the
        histogram.
  ...
  def assertHistogramsAllClose(self, x_keys, x_values, y_keys, y_values):
    ...
    """
    histogram_x = self.create_np_histogram(x_keys, x_values)
    histogram_y = self.create_np_histogram(y_keys, y_values)
